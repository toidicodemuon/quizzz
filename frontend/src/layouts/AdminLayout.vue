<template>
  <div class="min-h-screen flex">
    <aside class="w-64 bg-base-200 p-4 space-y-2">
      <div class="text-xl font-bold mb-4">Quiz Admin</div>
      <RouterLink class="btn btn-ghost justify-start w-full" to="/admin/users">{{ $t('nav.users') }}</RouterLink>
      <RouterLink class="btn btn-ghost justify-start w-full" to="/admin/quizzes">{{ $t('nav.quizzes') }}</RouterLink>
    </aside>
