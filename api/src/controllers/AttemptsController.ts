...
export class AttemptController extends Controller {
  ...
  public async listSubmissions(
    ...
    if (role === "ADMIN") {
      ...
      const [raw, total] = await Promise.all([
        prisma.attempt.findMany({
          ...
          select: {
            ...
            status: true,
            student: {
              select: { fullName: true, email: true, userCode: true },
            },
            exam: {
      ...
      for (const g of correctGroups as any)
        correctMap.set(g.attemptId, Number(g._count._all || 0));
      const totalMap = new Map<number, number>();
      for (const g of totalGroups as any)
        totalMap.set(g.examId, Number(g._count._all || 0));
      const answerCountMap = new Map<number, number>();
      for (const g of answerGroups as any)
        answerCountMap.set(g.attemptId, Number(g._count._all || 0));
      ...
      const items = raw.map((i) => ({
        ...
        pass:
          typeof (i as any).exam?.passMarkPercent === "number" &&
          i.score !== null
            ? Number(i.score) >= Number((i as any).exam.passMarkPercent)
            : null,
    ...
    const items = raw.map((i) => ({
      ...
      totalQuestions: totalMap.get(i.examId) || 0,
      pass:
        typeof (i as any).exam?.passMarkPercent === "number" && i.score !== null
          ? Number(i.score) >= Number((i as any).exam.passMarkPercent)
          : null,
      answerCount: answerCountMap.get(i.id) ?? 0,
    })) as any;
  ...
  public async getDetail(
    ...
    const sub = await prisma.attempt.findUnique({
      ...
      select: {
        ...
        status: true,
        exam: {
          select: { authorId: true, title: true, passMarkPercent: true },
        },
        student: { select: { fullName: true, email: true, userCode: true } },
      },
    });
    ...
    const pts = examQuestions.map((eq) => ({
    ...
    return {
      ...
      answers: examQuestions.map((eq) => {
        ...
        const earned =
          a?.earned === null || typeof a?.earned === "undefined"
            ? 0
            : Number(a?.earned as any);
        return {
          questionId: eq.questionId,
  ...
  public async createSubmission(
    ...
    for (const sel of submittedAnswers) {
      ...
      if (!found) {
        this.setStatus(400);
        throw new Error(
          `Invalid selectedChoiceId for question ${sel.questionId}`
        );
      }
    ...
    const totalPoints = await prisma.examQuestion.aggregate({
      _sum: { points: true },
