...
export default defineComponent({
  ...
  setup() {
      ...
      try {
        const [all, pub, draft, subs] = await Promise.all([
          ...
          ApiService.query("quizzes", {
            params: { teacherId, status: "PUBLISHED", pageSize: 1 },
          }),
          ApiService.query("quizzes", {
            params: { teacherId, status: "DRAFT", pageSize: 1 },
          }),
        ...
        latestSubmissions.value = Array.isArray(subs.data?.items)
          ? subs.data.items
          : [];
      } catch (e) {
        // ignore for now
    ...
    return { metrics, latestSubmissions, loading };
  },
});
</script>
